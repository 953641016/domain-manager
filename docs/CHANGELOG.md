...
### 修复
- **飞书卡片回调在线状态误判**（`backend/app/api/v1/feishu.py`）：卡片交互回调除兼容请求体 `token` 外，新增 `X-Lark-Request-Timestamp`、`X-Lark-Request-Nonce`、`X-Lark-Signature` 请求头签名校验，避免新版 `card.action.trigger` 回调未携带 body token 时被后端 403，导致飞书客户端提示“目标回调服务当前未在线”。
