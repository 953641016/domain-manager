...
- **Cloudflare 权限错误提示**（`backend/app/adapters/cloudflare.py`）：Cloudflare 重定向规则接口返回 `Authentication error` 时，提示补充 `Dynamic URL Redirects Write` 权限和当前 Zone 范围，避免只展示原始错误码。
