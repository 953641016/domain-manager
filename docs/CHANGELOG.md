...
- **域名购买报价可见性**（`backend/app/api/v1/feishu.py`）：购买域名审批卡片正文新增完整“服务商报价”列表，下拉选项改为价格/状态前置，避免飞书下拉选中态截断后看不到报价。
- **注册商下拉展示收敛**（`backend/app/api/v1/feishu.py`）：域名购买审批卡片下拉选项改为“注册商 | 特征账号”格式，账号只展示邮箱前缀等高识别片段；报价统一保留在上方服务商报价区，下方补充默认选择说明。
