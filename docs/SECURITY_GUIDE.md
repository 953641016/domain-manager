...
2. [超级管理员配置](#2-超级管理员配置)
3. [安全加固步骤](#3-安全加固步骤)
4. [域名转入转出功能说明](#4-域名转入转出功能说明)
5. [部署安全检查清单](#5-部署安全检查清单)
6. [安全事件响应](#6-安全事件响应)
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 域名转入 | ❌ 已禁用 | 代码保留但未暴露API |
| 获取转移码（转出） | ❌ 已禁用 | 代码保留但未暴露API |
...
为保持系统灵活性，**功能代码已保留**但**API层未暴露**：

| 文件 | 内容 |
|------|------|
| `adapters/base.py` | 基类方法含废弃注释 |
| `adapters/cloudflare.py` | Cloudflare转入实现 |
| `adapters/godaddy.py` | GoDaddy转入和获取转移码实现 |
| `registrar_factory.py` | `supports_transfer` 设为 `False` |

### 4.4 如何重新启用

如需启用域名转入转出功能，需完成以下安全评估：
...
1. **权限设计** (`permission.py`)
   ```python
   # 添加转入转出权限
   "can_transfer_in": True,   # 转入域名
   "can_transfer_out": True,  # 转出域名
   ```
